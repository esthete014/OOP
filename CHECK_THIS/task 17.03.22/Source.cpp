            ...
            case 1:
                if (_inputType == bySides) {
                    ...
                    if (h > 0 || AB > 0) {
                        S = (h * AB) / 2;
                        /*Point Triangle_this;
                        Triangle_this.c[0] = AB;
                        Triangle_this.b[0] = AB / 2;
                        Triangle_this.b[1] = h;
                        S = ((Triangle_this.b[0] - Triangle_this.a[0]) * (Triangle_this.c[1] - Triangle_this.a[1]) - (Triangle_this.c[0] - Triangle_this.a[0]) * (Triangle_this.b[1] - Triangle_this.a[1])) / 2;*/
                        std::cout << "Square of triangle: S = " << S << std::endl;
                    }
                    else {
                        std::cout << "Input error: triangle dont exist" << std::endl;
                    ...
                    /*Point Triangle_this;
                    for (int i = 0; i < 2; i++) {
                        Triangle_this.a[i] = A[i];
                        Triangle_this.b[i] = B[i];
                        Triangle_this.c[i] = C[i];
                    }
                    S = ((Triangle_this.b[0] - Triangle_this.a[0]) * (Triangle_this.c[1] - Triangle_this.a[1]) - (Triangle_this.c[0] - Triangle_this.a[0]) * (Triangle_this.b[1] - Triangle_this.a[1])) / 2;*/
                    //std::cout << "Square of triangle: S = " << S << std::endl;
                }
